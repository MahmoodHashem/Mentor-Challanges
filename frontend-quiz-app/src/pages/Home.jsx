...
export default function Home(props) {
    return (
        <div className="home font-rubik p-3 lg:px-24 lg:py-16 transition-all bg-[url(/pattern-background-mobile-light.svg)]  dark:bg-[url(/pattern-background-mobile-dark.svg)] md:bg-[url(/pattern-background-desktop-light.svg)] md:dark:bg-[url(/pattern-background-desktop-dark.svg)] bg-contain bg-no-repeat bg-light-grey dark:bg-dark-navy h-screen  w-screen">
             <nav className="flex lg:pb-5  p-5 gap-2 justify-end  ">
                <img src={props.theme === 'dark' ? "/icon-sun-light.svg" : "/icon-sun-dark.svg"} alt="icon-sun" />
                <label htmlFor="toggle" className='block rounded-2xl box-content border-4 border-custom-purple  w-toggle-width h-circle-size bg-custom-purple'>
                    <input onChange={props.handleTheme} type="checkbox" id="toggle" className="toggle-input hidden"></input>
                    <div className="toggle-switch transition-all  w-circle-size h-circle-size bg-white rounded-full"></div>
                </label>
                <img src={props.theme === 'dark' ? "/icon-moon-light.svg" : "/icon-moon-dark.svg"} alt="" />
            </nav>
